  ...
  postTimeout = setTimeout(() => {
    ...
    if (companyEle) {
      if (infiltrationStart) {
        console.info(
          `FAILED INFILTRATION - ${((new Date().valueOf() - infiltrationStart) / 1000).toFixed(1)} sec, last was ${
            state.lastCompany
          }`,
        );
        infiltrationStart = 0;
...
function acceptMoney(msg) {
  ...
  postTimeout = setTimeout(() => {
    ...
    if (btn) {
      if (infiltrationStart) {
        console.info(
          `SUCCESSFUL INFILTRATION - ${((new Date().valueOf() - infiltrationStart) / 1000).toFixed(1)} sec: ${
            btn.innerText
          }`,
        );
        infiltrationStart = 0;

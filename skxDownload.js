...
export function autocomplete(data, args) {
  ...
  if (['--download', '--subfolder', '--omit-folder'].includes(lastFlag)) return data.scripts;
  return [];
}
...
export async function main(ns) {
  ...
  for (const localFilePath of filesToDownload) {
    ...
    if (
      (await ns.wget(`${remoteFilePath}?ts=${new Date().getTime()}`, fullLocalFilePath)) &&
...
async function repositoryListing(ns, folder = '') {
  ...
  try {
    ...
    for (const folder of folders) files = files.concat((await repositoryListing(ns, folder)).map((f) => `/${f}`)); // Game requires folders to have a leading slash
    return files;
  } catch (error) {

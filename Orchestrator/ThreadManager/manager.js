...
import { DEBUG } from "/Orchestrator/Config/Debug.js";
export class Thread {
  host;
  inUse;
  locked;
...
export async function main(ns) {
  ...
  async function getAvailableThreads(message) {
    ...
    if (threads.length) {
      const availableThreads = threads.filter((thread) => !thread.inUse && !thread.locked).length;
  ...
  async function getThreads(message) {
    ...
    await messageHandler.sendMessage(
      message.origin,
      new Payload(Action.threads, allocatedThreadsByHost),
      message.originId
    );
  }
  async function freeThreads(message) {
    ...
    for (const host of Object.keys(threadsInfo)) {
      ...
      const usedThreads = threads.filter(usedThreadFilter);
      threads = threads.filter((t) => !usedThreadFilter(t));
      const threadsToRelease = usedThreads.splice(0, threadsInfo[host]);
      threads.push(...usedThreads);
  ...
  async function consoleThreadsUse(message) {
    for (const host of [...new Set(threads.map((thread) => thread.host))]) {
      ...
      ns.tprint(
        host + " ".repeat(padding) + ": [" + barSymbol.repeat(numberOfBar) + dashSymbol.repeat(numberOfDash) + "] (" + hostThreadsInUse.length + "/" + hostThreads.length + ")  " + hostUsedRam + " GiB/" + hostMaxRam + " GiB"
      );
    }
  }

...
import { Payload } from "/Orchestrator/MessageManager/class.js";
import { KILL_MESSAGE } from "/Orchestrator/Config/Config.js";
import { freeThreads } from "/Orchestrator/ThreadManager/common.js";
...
export async function executeScript(ns, script, threads, hack, messageHandler, id) {
  ...
  for (const host of Object.keys(threads)) {
    ...
    if (pid > 0) {
      ...
      dprint(
        ns,
        "Hack " + id + " targeting " + hack.host + " could not start script on " + host + " with " + threads[host] + " threads."
      );

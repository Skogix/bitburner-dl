...
export async function main(ns) {
  ...
  while (true) {
    try {
      ...
      log(
        ns,
        `WARNING: stats.js Caught (and suppressed) an unexpected error in the main loop. Update Skipped:\n` +
...
function updateHudElement(header, visible, value, toolTip) {
  const id = makeID(header),
    ...
    maybeUpdate = (id, index, value) => {
      if (nodeMap[id][index].textContent != value) nodeMap[id][index].textContent = value;
    };
...
async function getHudData(ns, bitNode, dictSourceFiles, options) {
  ...
  // Show what bitNode we're currently playing in
  {
    const val = [
      'BitNode',
      true,
      `${bitNode}.${1 + (dictSourceFiles[bitNode] || 0)}`,
      `Detected as being one more than your current owned SF level (${
        dictSourceFiles[bitNode] || 0
      }) in the current bitnode (${bitNode}).`,
    ];
    hudData.push(val);
  }
  ...
  // Show Hashes
  {
    const val1 = ['Hashes'];
    const val2 = [' '];
    if (9 in dictSourceFiles || 9 == bitNode) {
      // Section not relevant if you don't have access to hacknet servers
      const hashes = await getNsDataThroughFile(
        ns,
      ...
      if (hashes[1] > 0) {
        val1.push(
          true,
          `${formatNumberShort(hashes[0], 3, 1)}/${formatNumberShort(hashes[1], 3, 1)}`,
      ...
      if (
        ns.isRunning('spend-hacknet-hashes.js', 'home', '--liquidate') ||
        ns.isRunning('spend-hacknet-hashes.js', 'home', '-l')
      ) {
        val2.push(
          true,
          'Liquidating',
          'You have a script running that is selling hashes as quickly as possible (likely `spend-hacknet-hashes.js --liquidate`)',
        );
  ...
  hudData.push([
    'Scr Inc',
    true,
    formatMoney(ns.getTotalScriptIncome()[0], 3, 2) + '/sec',
    "Total 'instantenous' income per second being earned across all scripts running on all servers.",
  ]);
  hudData.push([
    'Scr Exp',
    true,
    formatNumberShort(ns.getTotalScriptExpGain(), 3, 2) + '/sec',
    "Total 'instantenous' hack experience per second being earned across all scripts running on all servers.",
  ]);
  ...
  // Show reserved money
  {
    ...
    if (reserve > 0) {
      val.push(
        true,
        formatNumberShort(reserve, 3, 2),
        'Most scripts will leave this much money unspent. Remove with `run reserve.js 0`',
      );
    } else val.push(false);
    hudData.push(val);
  }
  ...
  // Show gang income and territory
  {
    ...
    if ((2 in dictSourceFiles || 2 == bitNode) && gangInfo) {
      ...
      val1.push(
        true,
        formatMoney(gangInfo.moneyGainRate * 5, 3, 2) + '/sec',
      ...
      val2.push(
        true,
        formatNumberShort(gangInfo.territory * 100, 4, 2) + '%',
        `How your gang is currently doing in territory warfare. Starts at 14.29%\n` +
          ...
          `Power: ${gangInfo.power.toLocaleString('en')}  Clash ${
            gangInfo.territoryWarfareEngaged ? 'enabled' : 'disabled'
          } ` +
  ...
  // Show Karma if we're not in a gang yet
  {
    ...
    if (
      ...
      val.push(
        true,
        karmaShown,
        "After Completing BN2, you need -54,000 Karma in other BNs to start a gang. You also need a tiny amount to join some factions. The most is -90 for 'The Syndicate'",
      );
    } else val.push(false);
    hudData.push(val);
  }
  ...
  // Show number of kills if explicitly enabled
  {
    ...
    if (options['show-peoplekilled']) {
      ...
      val.push(
        true,
        formatSixSigFigs(numPeopleKilled),
        "Count of successful Homicides. Note: The most kills you need is 30 for 'Speakers for the Dead'",
      );
    } else val.push(false);
    hudData.push(val);
  }
  ...
  // Show Bladeburner Rank and Skill Points
  {
    ...
    if (
      (7 in dictSourceFiles || 7 == bitNode) &&
  ...
  // Show various server / RAM utilization stats
  {
    ...
    if (!options['hide-RAM-utilization']) {
      ...
      val1.push(
        true,
        `${servers.length}/${rooted}/${purchased}`,
        `The number of servers on the network (${servers.length}) / ` +
          `number rooted (${rooted}) / number purchased ` +
          (likelyHacknet.length > 0
            ? `(${purchased - likelyHacknet.length} servers + ${likelyHacknet.length} hacknet servers)`
            : `(${purchased})`),
      );
      ...
      val2.push(
        true,
        `${formatRam(home.maxRam)} ${((100 * home.ramUsed) / home.maxRam).toFixed(1)}%`,
      ...
      const [totalMax, totalUsed] = servers
        .filter((s) => s.hasAdminRights && (includeHacknet || !s.hostname.startsWith('hacknet-node-')))
        .reduce(([totalMax, totalUsed], s) => [totalMax + s.maxRam, totalUsed + s.ramUsed], [0, 0]);
      // Add Total Network RAM and Utilization
      val3.push(
        true,
        `${formatRam(totalMax)} ${((100 * totalUsed) / totalMax).toFixed(1)}%`,
        `Shows the sum-total RAM and utilization across all rooted hosts on the network` +
          (9 in dictSourceFiles || 9 == bitNode
            ? includeHacknet
              ? '\n(including hacknet servers, because you have scripts running on them)'
              : ' (excluding hacknet servers)'
            : '') +
          `\nDetails: Using ${formatRam(totalUsed)} of ${formatRam(totalMax)} (${formatRam(
            totalMax - totalUsed,
          )} free)`,
      );
    } else {
  ...
  // Show current share power
  {
    ...
    if (sharePower > 1.0001) {
      val.push(
        true,
        formatNumberShort(sharePower, 3, 2),

...
export function autocomplete(data, args): any {
 ...
 * @param {NS} ns **/
function tailAndLog(ns: NS, message: string) {
  ns.tail();
  log(ns, message, true);
}
...
export async function main(ns: NS) {
  ...
  if (options['enable-logging']) ns.tail();
  ...
  if (ns.getPlayer().city != 'Aevum') {
    try {
      if (
        ns.getPlayer().money < 200000 ||
        !(await getNsDataThroughFile(ns, 'ns.singularity.travelToCity(ns.args[0])', null, ['Aevum']))
      )
  ...
  const checkForFocusScreen = async () =>
    (await findRetry(ns, "//button[contains(text(), 'Stop playing')]", true))
      ? false // False positive, casino "stop" button, no problems here
  ...
  while (attempts++ <= 10) {
    ...
    try {
      ...
      if (btnStopAction) {
        // If we were performing an action unfocused, it will be focused on restart and we must stop that action to navigate.
      ...
      try {
        // Try to do this without SF4, because it's faster and doesn't require a temp script to be cleaned up below
        ...
        // Use SF4 as a fallback, it's more reliable.
        try {
          await getNsDataThroughFile(ns, 'ns.singularity.goToLocation(ns.args[0])', newFunction(), [
            'Iker Molina Casino',
          ]);
        } catch {
          return tailAndLog(
      ...
      if (!blackjack) {
        tailAndLog(
          ns,
          `ERROR: Could not find the "Play blackjack" button. Did something steal focus? Trying again... ` +
      ...
      if (!inputWager || !btnStartGame) {
        tailAndLog(
          ns,
          `ERROR: Could not find one or more game controls. Did something steal focus? Trying again... ` +
            `Please post a full-game screenshot on Discord if you can't get past this point.`,
        );
        continue; // Loop back to start and try again
      }
      ...
      if (ns.ls('home', 'Temp/').length > 0) {
        // Do a little clean-up to speed up save/load.
        ...
        if (await findRetry(ns, "//p[contains(text(), 'Count:')]", true, 10)) {
          ...
          // Otherwise, we've probably been kicked out of the casino, but...
        ...
        if (options['kill-all-scripts']) await killAllOtherScripts(ns, !options['no-deleting-remote-files']);
  ...
  while (true) {
    ...
    let won;
    do {
      // Inner-loop to play a single hand
      won = (await findRetry(ns, "//p[contains(text(), 'lost')]", true))
        ? false // Detect whether we lost or won. Annoyingly, when we win with blackjack, "Won" is Title-Case.
      ...
      if (won === null) {
        ...
        if (!txtCount) {
          // If we can't find the count, we've either been kicked out, or maybe routed to another screen.
...
async function killAllOtherScripts(ns: NS, removeRemoteFiles: boolean) {
  ...
  const serversExceptHome = allServers.filter((s: string) => s != 'home');
...
function onCompletion(ns: NS) {
  ...
  else log(ns, `WARNING: casino.js shutting down, but failed to launch ${completionScript}...`, false, 'warning');
}

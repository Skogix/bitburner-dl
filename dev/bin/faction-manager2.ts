import { AutocompleteData, NS, RunOptions } from '@ns';
export type huhuatsn = string;
import {
  log,
  getConfiguration,
  instanceCount,
...
const easyAccessFactions = [
  ...
  'Ishima',
  'Aevum',
  'Volhaven', // Location-Based
  'BitRunners',
  'CyberSec',
  'NiteSec',
  /* Hack Based */ 'Netburners',
  /* Hacknet-based */ 'Slum Snakes',
  'Tetrads' /* Early Crime */,
];
...
let favorToDonate: number | null; // Based on the current BitNode Multipliers, the favour required to donate to factions for reputation.
// Various globals because this script does not do modularity well
let playerData: Player = root.Player,
  bitNode = 0,
  gangFaction: string | number | null;
let augsAwaitingInstall: number,
  startingPlayerMoney: number | null,
  stockValue = 0; // If the player holds stocks, their liquidation value will be determined
let factionNames: string[] = [],
  joinedFactions: string | any[] = [],
  desiredStatsFilters: any[] = [],
  purchaseFactionDonations: { [s: string]: unknown } | ArrayLike<unknown> = [];
let ownedAugmentations = [],
  simulatedOwnedAugmentations: any[] = [],
  effectiveSourceFiles: any[] = [],
  allAugStats: any[] = [],
  priorityAugs: any[] = [],
  purchaseableAugs: any[] = [];
let factionData = {},
  augmentationData = {};
let printToTerminal: boolean | undefined, ignorePlayerData: any;
let _ns: NS; // Used to avoid passing ns to functions that don't need it except for some logs.

let options: RunOptions = null; // A copy of the options used at construction time
const argsSchema = [
...
export async function main(ns: NS) {
  ...
  priorityAugs = options['priority-aug']?.map((f: string) => f.replaceAll('_', ' '));
  ...
  let desiredAugs: name[] = options['aug-desired'].map((f: string) => f.replaceAll('_', ' '));
  ...
  if (sf4Level == 0) return log(ns, `ERROR: This script requires SF4 (singularity) functions to work.`, true, 'error');
  else if (sf4Level < 3)
    log(
      ns,
      `WARNING: This script makes heavy use of singularity functions, which are quite expensive before you have SF4.3. ` +
  ...
  ownedAugmentations = await getNsDataThroughFile(
    ns,
    'ns.singularity.getOwnedAugmentations(true)',
    '/Temp/player-augs-purchased.txt',
  );
  const installedAugmentations = await getNsDataThroughFile(
    ns,
  ...
  simulatedOwnedAugmentations = ignorePlayerData ? [] : ownedAugmentations.filter((a: string) => a != strNF);
  ...
  if ((desiredStatsFilters?.length ?? 0) == 0)
    ...
    desiredStatsFilters =
      ownedAugmentations.length > 40
        ? ['*'] // Once we have more than N augs, switch to buying up anything and everything
        : bitNode == 6 || bitNode == 7 || playerData.factions.includes('Bladeburners')
        ? ['*'] // If doing bladeburners, combat augs matter too, so just get everything
  ...
  await updateFactionData(
    ns,
    options['ignore-faction'].map((f) => f.replaceAll('_', ' ')),
  );
  ...
  if (ignorePlayerData) log(ns, 'INFO: Skipping joining available factions due to the --ignore-player-data flag set.');
  else {
    log(ns, 'Joining available factions...');
    const forceJoinFactions = options['force-join'] || [];
    if (!forceJoinFactions && !gangFaction && 2 in ownedSourceFiles && ns.heart.break() <= -53000) {
      ...
      log(
        ns,
        `INFO: Will join any gang faction because Karma is at ${formatNumberShort(ns.heart.break())}`,
        printToTerminal,
        printToTerminal ? 'info' : undefined,
      );
    }
    const joined = await joinFactions(ns, forceJoinFactions);
  ...
  if (
    options.purchase &&
    ownedAugmentations.length <= 1 &&
    13 in ownedSourceFiles &&
    !ownedAugmentations.includes(staneksGift) &&
    !options['ignore-stanek']
  )
    log(
      ns,
      `WARNING: You have not yet accepted Stanek's Gift from the church in Chongqing. Purchasing augs will ` +
        `prevent you from doing so for the rest of this BN. (Run with '--ignore-stanek' to bypass this warning.)`,
      true,
    );
  ...
  } else if (!ignorePlayerData)
    // Write a temp file that summarizes what augs we could afford if we could ascend right now.
    await ns.write(
      output_file,
      JSON.stringify({
...
function shorten(mult) {
  return mult
    ...
    .replace('hacking', 'hack')
    .replace('hacknet_node', 'hn')
    .replace('bladeburner', 'bb')
    .replace('stamina', 'stam')
    .replace('success_chance', 'success')
    .replace('success', 'prob')
    .replace('chance', 'prob');
}
...
function unshorten(strMult) {
  ...
  const match =
    stat_multis.find((m) => m == strMult || shorten(m) == strMult) || // Match exactly on the short-form of a multiplier
...
const factionSortOrder = (a, b) => factionSortValue(a) - factionSortValue(b);
const factionSortValue = (faction) => {
  const preferredIndex = factionNames.indexOf(faction.name || faction);
...
async function getSingularityDict(ns, command, listItems) {
  return await getNsDataThroughFile(
    ns,
    dictCommand(`ns.singularity.${command}(o)`),
    `/Temp/singularity-${command}-all.txt`,
    listItems,
  );
}
...
async function updateFactionData(ns, factionsToOmit) {
  ...
  factionNames.push(
    ...(options['hide-locked-factions'] ? easyAccessFactions : allFactions).filter((f) => !factionNames.includes(f)),
  );
  ...
  const dictFactionAugs = await getSingularityDict(ns, 'getAugmentationsFromFaction', factionNames);
  const dictFactionReps = await getSingularityDict(ns, 'getFactionRep', factionNames);
  const dictFactionFavors = await getSingularityDict(ns, 'getFactionFavor', factionNames);
      ...
      faction,
      {
        ...
        unownedAugmentations: function (includeNf = false) {
          return this.augmentations.filter(
            (aug) => !simulatedOwnedAugmentations.includes(aug) && (aug != strNF || includeNf),
          );
        },
        ...
        totalUnownedMults: function () {
          return this.unownedAugmentations()
            ...
            .reduce(
              (arr, aug) =>
                Object.keys(aug.stats).forEach((stat) => (arr[stat] = (arr[stat] || 1) * aug.stats[stat])) || arr,
              new Map(),
            );
        },
      },
...
async function updateAugmentationData(ns, desiredAugs) {
  ...
  augmentationData = Object.fromEntries(
    augmentationNames.map((aug) => [
      aug,
      {
        ...
        desired:
          desiredAugs.includes(aug) || // Mark as "desired" augs explicitly requested, or those with stats in the 'stat-desired' command line options
          desiredStatsFilters.includes('*') ||
          desiredStatsFilters.includes('_') || // Wildcards - all stats are desired (_ is for backwards compatibility when all stat names ended with '_mult')
          Object.entries(dictAugStats[aug]).some(
            ([k, v]) => v != 1 && desiredStatsFilters.some((filter) => k.includes(filter)),
          ),
        ...
        toString: function () {
          const factionColWidth = 16,
            augColWidth = 40,
            statsColWidth = 60;
          const statKeys = Object.keys(this.stats);
          const statsString =
            `Stats:${statKeys.length.toFixed(0).padStart(2)}` +
            (statKeys.length == 0
              ? ''
              : ` { ${statKeys
                  .map((prop) => shorten(prop) + ': ' + Math.round((this.stats[prop] + Number.EPSILON) * 100) / 100)
                  .join(', ')} }`);
          ...
          const augNameShort =
            this.displayName.length <= augColWidth - fCreep
              ? this.displayName
              : `${this.displayName.slice(0, Math.ceil(augColWidth / 2 - 3 - fCreep))}...${this.displayName.slice(
                  this.displayName.length - Math.floor(augColWidth / 2),
                )}`;
          return (
            `${this.desired ? '*' : ' '} Price: ${formatMoney(this.price, 4).padEnd(7)} ${
              this.price <= budget ? '✓' : '✗'
            }  ` +
            `Rep: ${formatNumberShort(this.reputation, 4).padEnd(6)} ${
  ...
  const propagateDesired = (aug) =>
    !aug.desired ||
    !aug.prereqs ||
    aug.prereqs.forEach((prereqName) => {
      const pa = augmentationData[prereqName];
      if (!pa)
        return log(
          ns,
  ...
  allAugStats = Object.values(augmentationData)
    .flatMap((aug) => Object.keys(aug.stats))
...
async function joinFactions(ns, forceJoinFactions) {
  ...
  if (joinedFactions.some((f) => manualJoin.includes(f))) manualJoin = [];
  ...
  log(
    ns,
    `${accessibleAugmentations.size} augmentations are already accessible from our ${joinedFactions.length} joined factions.`,
  );
  // Check for faction invitations
  const invitations = Object.values(factionData).filter((f) => f.invited);
  log(
    ns,
    `Outstanding invitations from ${invitations.length} factions: ${JSON.stringify(invitations.map((f) => f.name))}`,
  );
  let joined = 0;
  // Join all factions with remaining augmentations we care about
  for (const faction of invitations.sort(factionSortOrder)) {
    const unownedAugs = faction.unownedAugmentations(true); // Filter out augmentations we've already purchased
    const newAugs = unownedAugs.filter((aug) => !accessibleAugmentations.has(aug)); //  Filter out augmentations we can purchase from another faction we've already joined
    const desiredAugs = newAugs.filter((aug) => augmentationData[aug].desired); //  Filter out augmentations we have no interest in
    log(
      ns,
      `${faction.name} has ${faction.augmentations.length} augs, ${unownedAugs.length} unowned, ${newAugs.length} not offered by joined factions, ` +
        `${desiredAugs.length} with desirable stats` +
    ...
    if (manualJoin.includes(faction.name) && !forceJoinFactions.includes(faction.name))
      log(
        ns,
        `INFO: You have an invite from faction ${faction.name}, but it will not be automatically joined, ` +
          `because this would prevent you from joining some other factions.`,
        printToTerminal,
        printToTerminal ? 'info' : undefined,
      );
    else {
      ...
      if ((response = await getNsDataThroughFile(ns, `ns.singularity.joinFaction(ns.args[0])`, null, [faction.name]))) {
        ...
        joined++;
      } else log(ns, `ERROR: Error joining faction ${faction.name}. Response: ${response}`, false, 'error');
    }
  }
  return joined;
}
...
const getReqDonationForRep = (rep, faction) =>
  Math.ceil(
    (1e6 * Math.max(0, rep - (faction.name ? faction : factionData[faction]).reputation)) /
      playerData.mults.faction_rep,
  );
const getReqDonationForAug = (aug, faction) => getReqDonationForRep(aug.reputation, faction || aug.getFromJoined());

const getTotalCost = (augPurchaseOrder) =>
  augPurchaseOrder.reduce((total, aug, i) => total + aug.price * augCountMult ** i, 0);

const augSortOrder = (a, b) =>
...
function sortAugs(ns, augs = []) {
  ...
  const initialCost = getTotalCost(augs);
  let totalMoves = 0;
  for (let i = augs.length - 1; i > 0; i--) {
    ...
    while (i + moved + 1 < augs.length) {
      // See if promoting augs from below the batch to above the batch reduces the overall cost
      const testOrder = augs.slice(),
        moveIndex = i + moved + 1,
        insertionIndex = i - batchLengh + 1 + moved;
      testOrder.splice(insertionIndex, 0, testOrder.splice(moveIndex, 1)[0]); // Try moving it above the batch
      const newCost = getTotalCost(testOrder);
  ...
  const finalCost = getTotalCost(augs);
  if (totalMoves > 0)
    log(
      ns,
      `Cost reduced by ${formatMoney(initialCost - finalCost)} (from ${formatMoney(initialCost)} to ${formatMoney(
        finalCost,
      )}) by bubbling ${totalMoves} augs up above batches of dependencies.`,
    );
  return augs;
}
...
async function manageUnownedAugmentations(ns, ignoredAugs) {
  ...
  const reqDaedalusAugs =
    bitNodeMults.DaedalusAugsRequirement < 2
      ? Math.round(30 * bitNodeMults.DaedalusAugsRequirement)
      : bitNodeMults.DaedalusAugsRequirement;
  const outputRows = [
    `Currently have ${ownedAugmentations.length}/${reqDaedalusAugs} Augmentations required for Daedalus.`,
  ];
  const unownedAugs = Object.values(augmentationData).filter(
    (aug) => (!aug.owned || aug.name == strNF) && !ignoredAugs.includes(aug.name),
  );
  if (unownedAugs.length == 0)
    return log(
      ns,
      `All ${Object.keys(augmentationData).length} augmentations are either owned or ignored!`,
      printToTerminal,
    );
  const unavailableAugs = unownedAugs.filter((aug) => aug.getFromJoined() == null);
  ...
  outputRows.push(
    legendTitle,
    '-'.repeat(legendTitle.length),
    '✓  Can afford',
    '✗  Cannot afford',
    '$  Can donate for rep',
    `*  Desired aug/stats (${desiredStatsFilters.join(', ')})`,
    '-'.repeat(legendTitle.length),
  );
  ...
  availableAugs = ignorePlayerData
    ? unavailableAugs // Note: We omit NF from available augs here because as many as we can afford are added at the end.
    : await manageFilteredSubset(
        ns,
        outputRows,
        'Available',
        availableAugs.filter((aug) => aug.name != strNF),
        true,
      );
  if (countAvailable > 0) {
    let augsWithRep = availableAugs.filter(
      (aug) => aug.canAfford() || (aug.canAffordWithDonation() && !options['disable-donations']),
    );
    ...
    const accessibleAugs = await manageFilteredSubset(
      ns,
      outputRows,
      'Desired Within Rep',
      augsWithRep.filter((aug) => aug.desired),
    );
  ...
  if (purchaseableAugs.length > 0)
    log(
      ns,
      `INFO: The above ${purchaseableAugs.length} augmentations ${options.purchase ? 'will' : 'can'} be purchased ` +
        `${stockValue > 0 ? 'after liquidating stocks' : 'right now'}.` +
        (options.purchase ? '' : ' Run with the --purchase flag to make the purchase.'),
      printToTerminal,
    );
}
...
function getCostString(augCost, repCost) {
  return (
    `${formatMoney(augCost + repCost, 4)}` +
    (repCost == 0 ? '' : ` (Augs: ${formatMoney(augCost, 4)} + Rep: ${formatMoney(repCost, 4)})`)
  );
}
...
function filterMissingPrereqs(ns, subset) {
  ...
  do {
    ...
    for (const aug of subset) {
      const missingPreqs = aug.prereqs.filter(
        (prereq) => !(simulatedOwnedAugmentations.includes(prereq) || subset.some((a) => a.name === prereq)),
      );
      if (missingPreqs.length > 0) {
        log(
          ns,
          `INFO: Removing from aug "${aug.name}" (${aug.getFromAny}) due to prerequisites having been filtered out: ${missingPreqs}`,
        );
 ...
 *  */
async function manageFilteredSubset(
  ns,
  outputRows,
  subsetName,
  subset,
  printList = undefined,
  removeMissingPrereqs = true,
  reorder = true,
) {
  ...
  if (removeMissingPrereqs) filterMissingPrereqs(ns, subset);
  ...
  const subsetSorted = reorder ? sortAugs(ns, subset.slice()) : subset;
  const [repCostByFaction, totalRepCost, totalAugCost] = computeCosts(subsetSorted);
  // By default, if the purchase order is unchanged after filtering out augmentations, don't bother reprinting the full list
  if (
    printList === true ||
    (printList !== false && options['show-all-purchase-lists'] && !subset.every((v, i) => v == subsetSorted[i]))
  )
    outputRows.push(
      `${subset.length} ${subsetName} Augmentations in Optimized Purchase Order:\n  ${subsetSorted.join('\n  ')}`,
    );
  outputRows.push(
    `Total Cost of ${subset.length} ${subsetName}:`.padEnd(37) +
      ` ${getCostString(totalAugCost, totalRepCost)}` +
      (totalRepCost == 0
        ? ''
        : `  Donate: {${Object.keys(repCostByFaction)
            .map((f) => `"${f}":${formatNumberShort(repCostByFaction[f], 4)}`)
            .join(', ')}}`),
  );
  return subsetSorted;
}
...
async function managePurchaseableAugs(ns, outputRows, accessibleAugs) {
  ...
  do {
    // Outer loop is only repeated if we have to drop a priority aug and start over with our purchasable augs determination
    restart = false; // Flag as to whether we need to loop again with different starting set of priority augs
    dropped = [];
    purchaseableAugs = filterMissingPrereqs(
      ns,
      accessibleAugs
        .slice()
        .filter((a) => !droppedPriorityAugs.includes(a.name) && a.price + getReqDonationForAug(a) <= budget),
    );
    ...
    while (totalAugCost + totalRepCost > budget && purchaseableAugs.length > 0) {
      const mostExpensiveAug = purchaseableAugs
        .filter((a) => !priorityAugs.includes(a.name))
        .slice()
        .sort((a, b) => b.price - a.price)[0];
      if (!mostExpensiveAug) {
        // If there is nothing but "priority augs" left, then we need the user to deprioritize one or the other
        const aPa = priorityAugs.filter((name) => purchaseableAugs.some((a) => a.name == name));
        const toDrop = aPa[aPa.length - 1];
        log(
          ns,
          `WARNING: We can afford ${aPa.length} priority augs on their own, but not together. We must drop the lowest-priority one: ${toDrop}`,
          true,
          'warning',
        );
      ...
      const costBefore = getCostString(totalAugCost, totalRepCost);
      purchaseableAugs = sortAugs(
        ns,
        purchaseableAugs.filter((aug) => aug !== mostExpensiveAug),
      );
      [purchaseFactionDonations, totalRepCost, totalAugCost] = computeCosts(purchaseableAugs);
      const costAfter = getCostString(totalAugCost, totalRepCost);
  ...
  const nextUpAug =
    dropped.length == 0
      ? null
      : `Next desired aug available at:`.padEnd(37) +
        ` ${dropped[0].costBefore}  ` +
        `for \"${dropped[0].aug.name}\" from "${dropped[0].aug.getFromJoined()}" (cheapest of ${
          dropped.length
        } dropped augs)`;
  ...
  if (augsAwaitingInstall > 0)
    outputRows.push(
      `WARNING: Prices all have a x ${formatNumberShort(augCountMult ** augsAwaitingInstall)} cost penalty, because ` +
  ...
  if (inaccessiblePriorityAugs.length > 0)
    outputRows.push(
      `INFO: ${inaccessiblePriorityAugs.length} 'priority' augs are not yet accessible: ${inaccessiblePriorityAugs
        .map((n) => `"${n}"`)
        .join(', ')}`,
    );
  ...
  if (additionalDroppedPri.length > 0)
    outputRows.push(
      `INFO: ${additionalDroppedPri.length} 'priority' augs had to be droped: ${additionalDroppedPri
        .map((n) => `"${n}"`)
        .join(', ')}`,
    );
  ...
  augNf.getFromJoined = function () {
    // NOTE: Must be a function (not a lambda) so that `this` is bound to the augmentation object.
    ...
    return this.joinedFactionsWithAug().sort(
      (a, b) => (b.donationsUnlocked ? 1 : 0) - (a.donationsUnlocked ? 1 : 0) || b.reputation - a.reputation,
    )[0]?.name;
  };
  ...
  if (!augNf.canAfford() && !augNf.canAffordWithDonation()) {
    outputRows.push(
      `Cannot purchase any ${strNF} because the next level requires ${formatNumberShort(
        augNf.reputation,
      )} reputation, but ` +
        (!getFrom
          ? `it isn't being offered by any of our factions`
          : `the best faction (${getFrom}) has insufficient rep (${formatNumberShort(
              factionData[getFrom].reputation,
            )}).`),
    );
    ...
    const factionsWithAug = Object.values(factionData)
      .filter((f) => f.augmentations.includes(augNf.name))
      .sort(factionSort);
    ...
    if (getFrom != factionsWithAug[0].name && factionsWithAug[0] != factionsWithAugAndInvite[0])
      outputRows.push(
        `SUGGESTION: Earn an invitation to faction ${factionsWithAug[0].name} to make it easier to get rep for ${strNF} since it has the most favor (${factionsWithAug[0].favor}).`,
      );
      ...
      outputRows.push(
        `Attempting to join faction ${factionWithMostFavor.name} to make it easier to earn rep for ${strNF} since it has the most favor (${factionWithMostFavor.favor}).`,
      );
      ...
      if (!joinedFactions.includes(factionWithMostFavor.name)) {
        ...
        if (invitedFactionsWithDonation.length > 0) {
          outputRows.push(
            `Failed to join ${
              factionWithMostFavor.name
            }. Attempting to join any factions with whom we have enough favour to donate: ${invitedFactionsWithDonation.join(
              ', ',
            )}.`,
          );
          joined = await joinFactions(ns, invitedFactionsWithDonation);
        } else outputRows.push(`Failed to join ${factionWithMostFavor.name}. NeuroFlux will not be accessible.`);
      }
  ...
  if (getFrom && !factionData[getFrom].donationsUnlocked) {
    ...
    if (factionData[getFrom].favor >= favorToDonate)
      outputRows.push(
        `WARNING: The current faction (${getFrom}) with the most rep for buying NeuroFlux levels does not support donating for reputation. ` +
  ...
  if (augNfFaction && (augNf.canAfford() || augNf.canAffordWithDonation()))
    log(
      ns,
      `Getting NF from faction ${augNfFaction.name} (rep: ${formatNumberShort(
        augNfFaction.reputation,
      )}). Cost of NF 1 is ` +
        `${formatMoney(augNf.price)}, requires reputation: ${formatNumberShort(augNf.reputation)} ` +
        `(have ${formatNumberShort(augNfFaction.reputation)}, donate ${formatNumberShort(
          getReqDonationForRep(augNf.reputation, augNfFaction),
        )})`,
    );
  let nextUpNf; // Will tell the user when they will unlock the next NF level
  while (augNfFaction && nfPurchased < 200) {
    // Limit to 200 to avoid breaking the game if near infinite money.
    ...
    const nextNfTotalRepDonation =
      nextNfRep <= augNfFaction.reputation ? 0 : getReqDonationForRep(nextNfRep, augNfFaction);
    ...
    log(
      ns,
      `NF ${String(nfPurchased + 1).padStart(2)} Requires ${formatNumberShort(nextNfRep, 4)} Reputation, ` +
    ...
    if (totalCostWithNextNf > budget || (nextNfRep > augNfFaction.reputation && !augNfFaction.donationsUnlocked)) {
      nextUpNf =
        `Next NF (${nfPurchased + 1}) will be available at:`.padEnd(37) +
        ` ${getCostString(totalAugCost + nextNfCost, totalRepCost + nextNfRepCost)}  Money (` +
        `${totalCostWithNextNf > budget ? '✗' : '✓'}) and ${formatNumberShort(nextNfRep)} Reputation with "${
          augNfFaction.name
        }" (` +
        ...
        (nextNfRep <= augNfFaction.reputation
          ? ''
          : augNfFaction.donationsUnlocked
          ? ', can donate'
  ...
  log(
    ...
    `With ${formatMoney(
      budget,
    )}, can afford to purchase ${nfPurchased} levels of ${strNF}. New total cost: ${getCostString(
      totalAugCost,
      totalRepCost,
    )}`,
  );
  ...
  manageFilteredSubset(
    ns,
    outputRows,
    `(${purchaseableAugs.length - nfPurchased} Augs + ${nfPurchased} NF)`,
    purchaseableAugs,
    true,
    false,
    false,
...
function computeAugsRepReqDonationByFaction(augmentations) {
  ...
  for (const aug of augmentations) {
    ...
    const fDonationsIndex = Object.keys(repCostByFaction).findIndex((f) => f == faction.name);
    const alternativeFaction = Object.keys(repCostByFaction).find(
      (f, i) =>
        f != faction.name &&
        factionData[f].augmentations.includes(aug.name) &&
        ((repCostByFaction[f] >= reqDonation && (fDonationsIndex == -1 || i < fDonationsIndex)) || // We're donating the same or more to the other faction, and were planning on donating to it before this one
          getReqDonationForAug(aug, f) - repCostByFaction[f] < reqDonation - (repCostByFaction[faction.name] || 0)),
    ); // The amount we've committed to donating the other faction is closer to this requirement
    if (alternativeFaction) {
      log(
        _ns,
        `INFO: Using alternative faction "${alternativeFaction}" for "${aug.name}" rather than earlier faction "${faction.name}"`,
      );
    ...
    if (reqDonation > 0) repCostByFaction[faction.name] = Math.max(repCostByFaction[faction.name] || 0, reqDonation);
  }
...
async function purchaseDesiredAugs(ns) {
  const totalRepCost = Object.values(purchaseFactionDonations).reduce((t, r) => t + r, 0);
  const totalAugCost = getTotalCost(purchaseableAugs);
  // Refresh player data to get an accurate read of current money
  playerData = await getNsDataThroughFile(ns, 'ns.getPlayer()');
  if (stockValue > 0)
    return log(
      ns,
      `ERROR: For your own protection, --purchase will not run while you are holding stocks (current stock value: ${formatMoney(
        stockValue,
      )}). ` +
        `Liquidate your shares before running (run stockmaster.js --liquidate) or run this script with --ignore-stocks to override this.`,
      printToTerminal,
      'error',
    );
  if (totalAugCost + totalRepCost > playerData.money && totalAugCost + totalRepCost > playerData.money * 1.1)
    // If we're way off affording this, something is probably wrong
    return log(
      ns,
      `ERROR: Purchase order total cost (${getCostString(totalAugCost, totalRepCost)})` +
        ` is far more than current player money (${formatMoney(
          playerData.money,
        )}). Your money may have recently changed (It was ${formatMoney(startingPlayerMoney)} at startup), ` +
        `or there may be a bug in purchasing logic.`,
      printToTerminal,
      'error',
    );
  if (totalAugCost + totalRepCost > playerData.money)
    // If we're just a little off affording this, it could be because a bit of money was just spent? Just warn and buy what we can
    log(
      ns,
      `WARNING: Purchase order total cost (${getCostString(totalAugCost, totalRepCost)})` +
        ` is a bit more than current player money (${formatMoney(
          playerData.money,
        )}). Did something else spend some money? ` +
        `(We had ${formatMoney(startingPlayerMoney)} at startup). Will proceed with buying most of the purchase order.`,
      printToTerminal,
      'warning',
    );
  ...
  if (Object.keys(purchaseFactionDonations).length > 0 && Object.values(purchaseFactionDonations).some((v) => v > 0)) {
    if (
      await getNsDataThroughFile(
        ns,
        'JSON.parse(ns.args[0]).reduce((success, o) => success && ns.singularity.donateToFaction(o.faction, o.repDonation), true)',
        '/Temp/facman-donate.txt',
        [
          JSON.stringify(
            Object.keys(purchaseFactionDonations).map((f) => ({
              faction: f,
              repDonation: purchaseFactionDonations[f],
            })),
          ),
        ],
      )
      ...
      log(
        ns,
        `SUCCESS: Donated to ${
          Object.keys(purchaseFactionDonations).length
        } factions to gain access to desired augmentations.`,
        printToTerminal,
        'success',
      );
    else
      log(
        ns,
        `ERROR: One or more attempts to donate to factions for reputation failed. Go investigate!`,
        printToTerminal,
        'error',
      );
  }
  ...
  const purchased = await getNsDataThroughFile(
    ns,
    'JSON.parse(ns.args[0]).reduce((total, o) => total + (ns.singularity.purchaseAugmentation(o.faction, o.augmentation) ? 1 : 0), 0)',
    '/Temp/facman-purchase-augs.txt',
  ...
  if (purchased == purchaseableAugs.length)
    ...
    log(
      ns,
      `ERROR: We were only able to purchase ${purchased} of our ${purchaseableAugs.length} augmentations. ` +
        `Expected cost was ${getCostString(totalAugCost, totalRepCost)}. Player money was ${formatMoney(
          playerData.money,
        )} right before purchase, ` +
        `is now ${formatMoney(await getNsDataThroughFile(ns, 'ns.getPlayer().money'))}`,
      printToTerminal,
      'error',
    );
}
...
function displayJoinedFactionSummary(ns) {
  const joinedFactions = Object.values(factionData).filter((f) => f.joined);
  let summary = `${joinedFactions.length} Joined Factions:`;
  const noaugs = joinedFactions.filter((f) => f.unownedAugmentations().length == 0);
  if (noaugs.length > 0)
    summary += `\n  ${noaugs.length} joined factions have no unowned augs remaining: "${noaugs
      .map((f) => f.name)
      .join('", "')}"`;
  for (const faction of joinedFactions.filter((f) => !noaugs.includes(f)))
    summary += `\n  ${faction.name}: ${faction.unownedAugmentations().length} augs remaining (${faction
      .unownedAugmentations()
      .join(', ')})`;
  log(ns, summary, printToTerminal);
}

/** @param {NS} ns **/
function displayFactionSummary(ns: NS, sortBy, unique, overrideFinishedFactions, excludedStats) {
  const noAugs = Object.values(factionData).filter((f) => f.unownedAugmentations().length == 0);
  let summary = '';
  if (noAugs.length > 0)
    summary += `${noAugs.length} factions have no augmentations to purchase (excluding NF): ${JSON.stringify(
      noAugs.map((a) => a.name),
    )}\n`;
  let summaryFactions = Object.values(factionData).filter(
    (f) => f.unownedAugmentations().length > 0 && !overrideFinishedFactions.includes(f.name),
  );
  ...
  const relevantAugStats = allAugStats.filter(
    (s) =>
      !excludedStats.find((excl) => s.includes(excl)) &&
      undefined !==
        summaryFactions.find((f) => f.unownedAugmentations().find((aug) => 1 != (augmentationData[aug].stats[s] || 1))),
  );
  summary += `${summaryFactions.length} factions with augmentations (✓=Joined ✉=Invited ✗=Locked, sorted by total ${sortBy}):`;
  // Creates the table header row
  const getHeaderRow = (countName) =>
  ...
  const getFactionSummary = (faction) => {
    const totalMults = faction.totalUnownedMults();
    return (
      `\n ${faction.joined ? '✓' : faction.invited ? '✉' : '✗'} ${faction.name} `.padEnd(32) +
      `${String(faction.unownedAugmentations().length).padStart(2)} / ${String(faction.augmentations.length).padEnd(
        2,
      )} ` +
      relevantAugStats
        .map((key) =>
          (totalMults[key] === undefined ? '-' : totalMults[key].toPrecision(3)).padStart(
            Math.max(shorten(key).length, 4),
          ),
        )
        .join(' ')
    );
  ...
  const sortFunction = (a, b) => {
    const aMultiContrib = a.totalUnownedMults()[sortBy] || 1,
      bMultiContrib = b.totalUnownedMults()[sortBy] || 1;
    const sort1 = bMultiContrib - aMultiContrib; // Sort by the total amount of desired multi provided by this faction
    const sort2 = (a.joined ? 0 : 1) - (b.joined ? 0 : 1); // If tied, sort by which faction we've joined
    ...
    const sort3 = b.reputation - a.reputation; // If tied, sort by which faction we have the most rep with
    if (sort3 != 0) return sort3;
    const sort4 = a.mostExpensiveAugCost().length - b.mostExpensiveAugCost().length; // If tied, "soonest to unlock", estimated by their most expensive aug cost
  ...
  const getSeparator = (faction) =>
    moreContributors && !(moreContributors = faction.totalUnownedMults()[sortBy] !== undefined)
  ...
  if (!unique)
    // Each faction is summarized based on all the unowned augs it has, regardless of whether a faction higher up the list has the same augs
    for (const faction of summaryFactions.sort(sortFunction))
      summary += getSeparator(faction) + getFactionSummary(faction);
  else {
    // Each faction's stats computed as though the faction sorted above it was joined and bought out first, so only showing new augs
  ...
  log(
    ns,
    `INFO: The following is a summary of ${unownedAugCount} remaining augmentations available from each faction:\n` +
      summary,
    printToTerminal,
  );
}
